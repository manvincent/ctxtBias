...
parfor s=1:length(subject_list); 
    ...
    if ~exist([subjectfolder filesep 'Signal_Cond_Ave_TrialBN'],'dir');
        mkdir([subjectfolder filesep 'Signal_Cond_Ave_TrialBN'])
    end
    ...
    TFRwave = [];;              
    TFRwave = load([subjectfolder filesep subject '_Signal_TrialBN.mat']);
    TFRwave = TFRwave.PreprocessedData;
    signal = cell2mat(TFRwave.trial);
    ...
    OO_L_stim_ind = [];
        for l = 1:length(EEG_Stim_Time)
            ...
            stim_time = [];
            if cell2mat(complete_sub_cell_noNA(l,16)) == 1 % Mask trials to include only odd trials (out-of-sample)
                if ~any(isnan(signal(1,knnsearch(TFRwave.timeround',EEG_Stim_Time(l))))) == 1
    ...
    OO_L_resp_ind = [];
        for l = 1:length(EEG_Resp_Time)
            ...
            resp_time = [];
            if cell2mat(complete_sub_cell_noNA(l,16)) == 1 
                if ~any(isnan(signal(1,knnsearch(TFRwave.timeround',EEG_Resp_Time(l))))) == 1
                    if strncmp(complete_sub_cell_noNA(l,5),'OF',2) && strncmp(complete_sub_cell_noNA(l,13),'Gain  ',6) == 1 % OF Context - Gains
    ...
    for c = 1:length(condition_list)
        ...
        for t = 1:length(curr_cond); 
            ...
            TFRtrial.(curr_cond_label).(trial_name) = signal(:,curr_cond_start(t):curr_cond_end(t));

            % Get the average across all trials within a condition
            trial_mat = [];
            trial_mat = TFRtrial.(curr_cond_label).(trial_name);
            if size(trial_mat,2) < 360
                trial_mat(:,360) = NaN;                    
            elseif size(trial_mat,2) > 360
                trial_mat(:,361) = [];
            end
        ...
        TFRtrial.(curr_cond_label).mean = nanmean(cat_trials,4);
        ave_cond = [];
        ...
        TFRcondave.fsample = TFRwave.fsample
        TFRcondave.time = prestim_length:temp_res:poststim_length;
        TFRcondave.trial = ave_cond;
        TFRcondave.elec = TFRwave.elec;
        TFRcondave.cfg = TFRwave.cfg;
        TFRcondave.cond = curr_cond_label;      

        parsave([subjectfolder filesep 'Signal_Cond_Ave_TrialBN' filesep curr_cond_label '_meanTFR_' event_lock '.mat'],1,TFRcondave,'-v7.3');
        else
            continue
        end
