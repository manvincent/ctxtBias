...
dv_name = [];
dv_name = char(full_table.Properties.VariableNames(2+1));            
% run single full_model for parameters
temp_model = fitlme(full_table, [dv_name model_description]);
...
for nCh = 1 : num_chans
    ...
    % get dependent variable name from model description
    dv_name = [];
    dv_name = char(full_table.Properties.VariableNames(2+nCh));
    % put the channel in the table
    full_table.(dv_name) = data_of_interest(nCh, :)';
    
    % run the actual model
    if FLAG_RANDOM
        full_model_object = fitlme(full_table, [dv_name model_description], ...
...
if NUM_PERMS > 0
    ...
    for n_perm = 1 : NUM_PERMS
        ...
        parfor nCh = 1 : num_chans
            ...
            %run the actual model
            full_model_object = fitlme(perm_table, [dv_name model_description], ...
                'covariancePattern', 'Diagonal'); % FullCholesky | CompSymm | Diagonal | Full
