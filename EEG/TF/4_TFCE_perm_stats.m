...
test_chan_list = {'TFCE_Res_Signal'};
%% Run Stats
for ev = 1:length(event_lock_list);
    curr_event = [];
    curr_event = char(event_lock_list(ev));
    
    for ch = 1:length(test_chan_list);
        curr_chan = [];
        curr_chan = char(test_chan_list(ch));
        fprintf('\n\n\n*** Testing averaged chan:  %s ***\n\n\n', curr_chan);
        ...
        % Load in group-compiled data
        if strncmp(curr_chan,'all',1) == 1 
            load([groupfolder filesep 'Group_Cond_' curr_event '_Cell.mat']);    
            ...
            load([groupfolder filesep 'Group_meanChan_' curr_chan '_Cond_' curr_event '_Cell.mat']);  
            FF_G = full_cond_meanchan_array{1,1};
            FF_C = full_cond_meanchan_array{2,1};
            FF_L = full_cond_meanchan_array{3,1};
            FO_G = full_cond_meanchan_array{4,1};
            FO_C = full_cond_meanchan_array{5,1};
            FO_L = full_cond_meanchan_array{6,1};
            OO_G = full_cond_meanchan_array{7,1};
            OO_C = full_cond_meanchan_array{8,1};
            OO_L = full_cond_meanchan_array{9,1};
            OF_G = full_cond_meanchan_array{10,1};
            OF_C = full_cond_meanchan_array{11,1};
            OF_L = full_cond_meanchan_array{12,1};
        end
        
        condition_stats_pvals_ME_ctxt = [];
        condition_stats_pvals_ME_stim = [];
        condition_stats_pvals_INTERAC = [];
        condition_stats_Fvals_ME_ctxt = [];
        condition_stats_Fvals_ME_stim = [];
        condition_stats_Fvals_INTERAC = [];
        condition_stats_TFCE_Fvals_ME_ctxt = [];
        condition_stats_TFCE_Fvals_ME_stim = [];
        condition_stats_TFCE_Fvals_INTERAC = [];
        
        condition_stats_TFCE_Pvals_ME_ctxt = [];
        condition_stats_TFCE_Pvals_ME_stim = [];
        condition_stats_TFCE_Pvals_INTERAC = [];
        ...
        [Info, Data, Results] = ept_TFCE_ANOVA(stats_data,loc,1)   ;
        ...
        parsave([statsout filesep 'Fvals_meanChan_' curr_chan '_' curr_event '_locked_ME_ctxt.mat'],1,condition_stats_Fvals_ME_ctxt,'-v7.3');
        parsave([statsout filesep 'Fvals_meanChan_' curr_chan '_' curr_event '_locked_ME_stim.mat'],1,condition_stats_Fvals_ME_stim,'-v7.3');
        parsave([statsout filesep 'Fvals_meanChan_' curr_chan '_' curr_event '_locked_INTERAC.mat'],1,condition_stats_Fvals_INTERAC,'-v7.3');             

        parsave([statsout filesep 'TFCE_Fvals_meanChan_' curr_chan '_' curr_event '_locked_ME_ctxt.mat'],1,condition_stats_TFCE_Fvals_ME_ctxt,'-v7.3');
        parsave([statsout filesep 'TFCE_Fvals_meanChan_' curr_chan '_' curr_event '_locked_ME_stim.mat'],1,condition_stats_TFCE_Fvals_ME_stim,'-v7.3');
        parsave([statsout filesep 'TFCE_Fvals_meanChan_' curr_chan '_' curr_event '_locked_INTERAC.mat'],1,condition_stats_TFCE_Fvals_INTERAC,'-v7.3');
        
        parsave([statsout filesep 'TFCE_Pvals_meanChan_' curr_chan '_' curr_event '_locked_ME_ctxt.mat'],1,condition_stats_TFCE_Pvals_ME_ctxt,'-v7.3');
        parsave([statsout filesep 'TFCE_Pvals_meanChan_' curr_chan '_' curr_event '_locked_ME_stim.mat'],1,condition_stats_TFCE_Pvals_ME_stim,'-v7.3');
        parsave([statsout filesep 'TFCE_Pvals_meanChan_' curr_chan '_' curr_event '_locked_INTERAC.mat'],1,condition_stats_TFCE_Pvals_INTERAC,'-v7.3');
