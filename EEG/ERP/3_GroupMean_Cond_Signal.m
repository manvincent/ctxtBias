...
for cond = 1:length(condition_list_label);
    ...
    for s=1:length(subject_list); 
        ...
        if exist([subjectfolder filesep 'Signal_Cond_Ave_TrialBN' filesep char(curr_cond) '_meanTFR_' event_lock '.mat'],'file');            

            fprintf('\n\n\n*** Condition compiling for condition %d, subject %d (%s) ***\n\n\n', cond, s, subject);

            % Load in subject's TFR data 
            TFRcondave = [];;              
            TFRcondave = load([subjectfolder filesep 'Signal_Cond_Ave_TrialBN' filesep char(curr_cond) '_meanTFR_' event_lock '.mat']);
            TFRcondave = TFRcondave.TFRcondave;


            % Compile all-channel condition matrices across subjects
            sub_data_mat = [];
            sub_data_mat = TFRcondave.trial;
...
% Save output
if ~exist([parentfolder filesep 'Group_Analysis_Signal'],'dir');
    mkdir([parentfolder filesep 'Group_Analysis_Signal']);
end
groupfolder = [parentfolder filesep 'Group_Analysis_Signal'];

parsave([groupfolder filesep 'Group_Signal_Cond_' event_lock '_Cell.mat'],1,full_cond_array,'-v7.3');
